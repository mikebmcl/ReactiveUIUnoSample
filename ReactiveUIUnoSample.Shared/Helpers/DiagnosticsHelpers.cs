...
namespace ReactiveUIUnoSample
{
    public static class DiagnosticsHelpers
    {
        ...
        /// <summary>
        /// Reports an issue via a unified mechanism. Will throw an <see cref="Exception"/> with the diagnostic message string it creates as its message and with <paramref name="innerException"/> as its inner exception when either <paramref name="severity"/> is <see cref="ProblemSeverity.Error"/> and <c>DEBUG</c> is defined or when <paramref name="severity"/> is <see cref="ProblemSeverity.CatastrophicFailure"/> regardless of other conditions. These are thrown in a finally block in case an exception occurs internally. So it is recommended that you catch any exception, and pass it to this using <paramref name="innerException"/> and then deal with it appropriately after calling this method (though you won't get there when this method (re)throws, as specified above). If <paramref name="severity"/> is <see cref="ProblemSeverity.Warning"/> and <c>DEBUG</c> is defined, <see cref="Debugger.Break"/> is called after the diagnostic message is called to enable examination of the stack, etc.
        /// </summary>
